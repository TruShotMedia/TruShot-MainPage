    ...
    <>
      ...
      <div className="pricing-admin-grid">
        {packages.map((item: Record<string, unknown>) => <article className={`pricing-admin-card ${item.is_featured ? "featured-admin" : ""}`} key={item.id as string}>
          ...
          <details className="inline-editor"><summary>Edit package</summary><form action={updatePricingPackage} className="quick-form"><input type="hidden" name="id" value={item.id as string} /><label>Title<input name="title" defaultValue={item.title as string} required /></label><label>Summary<textarea name="summary" defaultValue={item.summary as string} rows={4} required /></label><label>Price AUD<input name="price_dollars" type="number" min="0" step="0.01" defaultValue={Number(item.price_cents) / 100} /></label><label>Price suffix<input name="price_suffix" defaultValue={String(item.price_suffix ?? "")} /></label><button className="admin-primary-button" type="submit">Save & update website</button></form></details>
        </article>)}
      </div>
