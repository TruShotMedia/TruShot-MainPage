...
export const WEBSITE_MEDIA_ACCEPT = Object.keys(WEBSITE_MEDIA_EXTENSIONS).join(",");
...
export function validateWebsiteMediaFile(file: { type: string; size: number }) {
  ...
  if (!kind) {
    throw new Error("Choose an MP4 or WebM video, or a JPG, PNG, WebP or AVIF image.");
  }
