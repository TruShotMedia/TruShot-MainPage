...
export function getNotionConfiguration(): NotionConfiguration | null {
  ...
  const clientsSourceId = process.env.NOTION_CLIENTS_DATA_SOURCE_ID?.trim();
  ...
  if (!token || !clientsSourceId || !jobsSourceId || !tasksSourceId) return null;
...
export function getNotionConfigurationSummary() {
  return {
    configured: Boolean(
      ...
      && process.env.NOTION_CLIENTS_DATA_SOURCE_ID?.trim()
